...
def computeSnapshotCollaboration(file_commit, cmt_list, id_mgr, link_type,
    ...
    for cmt in revCmts:
        ...
        # irrelevant
        if cmt.id not in fileState_mod.values():
            continue
...
def compute_snapshot_collaboration_features(file_commit, cmt_list, id_mgr,
    ...
    for cmt in rev_cmts:
        ...
        # is now irrelevant
        if cmt.id not in file_state_mod.values():
            continue
...
def randomizeCommitCollaboration(codeBlks, fileState):
    ...
    (line number) where the commits were made. The idea behind this is to see if
    people really do make preferential attachment to people who are working on
    related code or the interactions seen from commit activity is random. If we
    see the no difference in the significance of the community structure between
    randomized and non randomized tests then we can assume interactions appear
    to be random. In other words the committers are not collaboration with
    people near their code anymore than someone making commits far from their
    code.
...
def simpleCluster(codeBlks, snapShotCmt, maxDist, author=False):
    ...
    for i in range(len(blksOfInterest) - 1):
        ...
        # block is too far away, save current cluster and start new cluster
        else:
    ...
    for blkIdx in otherBlks:
        ...
        if nextClusterIdx > finalClusterIdx:
            ...
            # block falls within this cluster
            if currClusterDist <= nextClusterDist:
                blkClusters[currClusterIdx].append(codeBlks[blkIdx])
...
def removePriorCommits(fileState, clist, startDate):
    """Removes commits that occurred prior to a 'startDate'

    Args:
        fileState (dict): A dict with code line numbers as keys and commit
            hashes for those lines as values
        clist (dict): Dict of Commit objects, mapping commit hashes to Commits.
...
def linesOfInterest(fileState, snapShotCommit, maxDist, cmt_list, file_commit):
    ...
    Args:
        ...
        snapShotCommit (str): the commit hash that marks when the fileState was
            acquired
    ...
    for lineNum, cmtId in fileState.items():
        if cmtId in cmt_list:
            cmtDate = cmt_list[cmtId].getCdate()
        else:
...
def createStatisticalData(cmt_list, id_mgr, link_type):
    ...
    Args:
        cmt_list (dict): Dict of Commit objects, mapping commit hashes to Commits.
        id_mgr (idManager): instance of idManager to handle person IDs and
...
def writeCommitData2File(cmt_list, id_mgr, outdir, releaseRangeID, dbm, conf,
    ...
    dbm.doExecCommit(
        "DELETE FROM commit WHERE projectId=%s AND releaseRangeId=%s",
    ...
    for cmt in cmt_list:
        ...
        values = {"commitHash": cmt.id,
                  ...
                  "Subsys": subsys_name,
                  "inRC": int(inRC),
                  "AuthorSubsysSimilarity": float(
                      cmt.getAuthorSubsysSimilarity()),
                  "AuthorTaggersSimilarity": float(
                      cmt.getAuthorTaggersSimilarity()),
                  "TaggersSubsysSimilarity": float(
                      cmt.getTaggersSubsysSimilarity()),
...
def writeDependsToDB(
    ...
    if get_entity_source_code is None:
        def get_source(file, id):
            ...
            Args:
                file (str): File name.
                id (int): Consecutive function ID inside the file.

            Returns:
                list: List of strings, one for each line of code.
    ...
    for entity_type_current, logical_depends_current in zip(entity_type,
        ...
        for cmt in cmt_list:
            ...
            # if there are dependencies for the current commit,
            # add them to the rows to be added to DB
            if (cmt.id in logical_depends_current) & (
                ...
                # get the corresponding source code (implementation) for all
                # dependencies and add it to the current dependency item
                depend_impl_list = [
                    ' '.join(get_entity_source_code(file, funcId))
                    for file, funcId in function_loc]
                depends_list = [
                    depends_list[indx][0] + (depends_list[indx][1], impl)
...
def writeAdjMatrix2File(id_mgr, outdir, conf):
    ...
    if link_type == LinkType.tag:
        for id_receiver in idlist:
            out.write("\t".join(
                [str(id_mgr.getPI(id_receiver).getActiveTagsReceivedByID(
                    id_sender).get_weight())
...
def emitStatisticalData(cmt_list, id_mgr, logical_depends, outdir,
    ...
    writeCommitData2File(cmt_list, id_mgr, outdir, releaseRangeID, dbm, conf)
    ...
    if logical_depends is not None:
        writeDependsToDB(logical_depends, cmt_list, dbm, conf, entity_type,
        ...
        for line_num in idx:
            ...
            if cmt_id in cmt_dict:
                ...
                if cmt_dict[cmt_id].getCdate() >= start_date:
                    ...
                    feature_expression_list = file.findFeatureExpression(
                        line_num)
...
def computeProximityLinks(fileCommitList, cmtList, id_mgr, link_type,
    ...
    for fileCommit in fileCommitList.values():
        ...
        if speedUp:
            ...
            # TODO This branch is defective and dead code!
...
def computeCommitterAuthorLinks(cmt_list, id_mgr):
    ...
    pbar = ProgressBar(widgets=widgets, maxval=len(cmt_list)).start()
    ...
    for i in range(0, len(cmt_list)):
        ...
        pi_committer.addSendRelation(LinkType.committer2author,
                                     pi_author.getID(), cmt, edge_weight)
        pi_author.addReceiveRelation(LinkType.committer2author,
                                     pi_committer.getID(), edge_weight)

    pbar.finish()
    # end for i


def computeTagLinks(cmt_list, id_mgr):
...
def performAnalysis(conf, dbm, dbfilename, git_repo, revrange, subsys_descr,
    ...
    emitStatisticalData(cmtlist, id_mgr, logical_depends, outdir,
                        releaseRangeID, dbm, conf, fileCommitDict, entity_type,
                        get_entity_source_code)
